...
# Importa a lógica de processamento em lote centralizada do batch_processor
...
class BatchTriggerHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        # Para diretórios, não tentamos abrir como arquivo (correção do bug de IOError).
        # Para PDFs, aguarda a liberação completa da escrita pelo sistema operacional.
        file_ready = event.is_directory  # Diretórios estão sempre prontos
        if not file_ready:
            retries = 5
            while retries > 0:
                try:
        ...
        if file_ready:
            ...
            try:
                ...
                run_batch(self.watch_dir, "batch_results.txt")
            finally:
